...
class RequestHeadersViewModel @Inject constructor(
    ...
    init {
        ...
        viewState = merge(
            intentSharedFlow.filterIsInstance<RequestHeadersIntent.Init>().take(1),
            intentSharedFlow.filterNot { it is RequestHeadersIntent.Init }
        )
    ...
    private fun SharedFlow<RequestHeadersIntent>.toRequestHeadersPartialStateChangeFlow(): Flow<RequestHeadersPartialStateChange> {
        return merge(
